...
export class TextAreaInput extends TextAreaInputEvents {
  ...
  onCompositionnStart(e: CompositionEvent) {
    ...
    if (
      ...
      const isArrowKey =
        ...
        (this._lastKeydown.keyCodeId === KeyboardEventCode.ARROW_RIGHT ||
          this._lastKeydown.keyCodeId === KeyboardEventCode.ARROW_LEFT);
  ...
  onFocus() {
    this.textarea.focus();
    console.log('onFocus', this.textarea, document.activeElement);
  ...
  #setHasFocus(hasFocus: boolean): void {
    console.log('set focus', hasFocus, this._hasFocus, document.activeElement);
  ...
  private _installSelectionChangeListener(): void {
    ...
    this._selectionChangeListener = on(
      ...
      if (!this._hasFocus || this._isDoingComposition || !isChrome()) return;
      const now = Date.now();
      const delta1 = now - previoursSelectionChangeEventTime;
      previoursSelectionChangeEventTime = now;
      if (delta1 < 5) return;
      const delta2 = now - this._textareaWrapper.getIgnoreSelectionChangeTime();
