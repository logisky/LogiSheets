...
export const TextContainerComponent = <T,>({
    ...
    const inputMng = useRef(
        new InputManager(textMng.current, selectionMng, context, cursorMng)
    )
    ...
    useEffect(() => {
        ...
        if (textElement) textMng.current.drawText(textElement)
        if (selectionElement) selectionMng.init(selectionElement)
        if (textareaElement) inputMng.current.init(textareaElement)
    }, [context])
    useEffect(() => {
        ...
        sub.add(
            on(window, EventType.MOUSE_UP).subscribe((mue) => {
                if (!isMouseDown.current) return
                mue.preventDefault()
        ...
        sub.add(
            textMng.current.textChanged().subscribe((t) => {
                type?.(t)
                suggestMng.onType()
            })
        )
    ...
    const onHostMouseDown = (mde: MouseEvent) => {
        mde.stopPropagation()
        mde.preventDefault()
        if (!inputMng.current.hasFocus()) inputMng.current.setFocus()
    ...
    const _onBlur = async () => {
        ...
        const event = new BlurEvent(context.bindingData)
        blur?.(event)
        return true
    }
