...
export class TokenManager {
    ...
    isOperandStart(token: Token) {
        if (token.type === TokenType.OPERAND) return true
        if (token.subtype === SubType.SEPARATOR) return true
        if (token.subtype === SubType.START) return true
        return false
    }
    getFnInfo(token: Token | number = -1) {
        ...
        let paramCount = 0
        let fnIndex = -1
        if (typeof token === 'object') tokenIndex = this.getTokenIndex(token)
        else tokenIndex = token
        if (tokenIndex === -1) return {fnIndex, paramCount}
        let stopCount = 1
    ...
    getTokenIndexByCursor(cursor: number, text: string) {
        ...
        for (let i = result; i >= 0; i--) {
            const t = tokens[i]
            if (t.type !== TokenType.WHITE_SPACE) break
            result = i
        }
        return result
    }
    getLastTokenWithType(
        currIndex: number,
        type: TokenType,
        subtype?: SubType
    ) {
        ...
        for (let i = currIndex; i >= 0; i--) {
            ...
            if (subtype !== undefined && t.subtype !== subtype) continue
            result = i
        }
        return result
    }
    getNextTokenWithType(
        currIndex: number,
        type: TokenType,
        subtype?: SubType
    ) {
    ...
    getTokenPosition(token: Token) {
        ...
        const start = this._tokens
            .slice(0, tokenIndex)
            .reduce((a, b) => a + b.value.length, 0)
