...
export const useResizers = (canvas: RefObject<HTMLCanvasElement>) => {
    ...
    const mousemove = (e: MouseEvent) => {
        if (!_active.current || !movingStart.current)
            return false
        const {isRow, leftCell: {position: {startCol, startRow, endCol, endRow}}} = _active.current
        let px = 0
        if (isRow) {
            const newEndRow = endRow + e.clientY - movingStart.current.y
            if (newEndRow > startRow)
                px = newEndRow - startRow
        } else {
            const newEndCol = endCol + e.clientX - movingStart.current.x
            if (newEndCol > startCol)
                px = newEndCol - startCol
        }
