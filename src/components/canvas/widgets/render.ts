...
export const useRender = () => {
  ...
  useEffect(() => {
    ...
    subs.add(
      canvasHost.obs().subscribe((data) => {
        if (data.type === 'yScroll') {
          const { oldScroll, newScroll } = data.args;
  ...
  const render = () => {
    const canvas = canvasHost.canvas;
    if (!canvas) throw Error('canvas not found');
    painterSvc.current.setupCanvas(canvas);
  ...
  const _border = (box: Box, position: Range, style?: StandardStyle) => {
    ...
    if (border.bottom) painterSvc.current.border(border.bottom, box, 'bottom');
