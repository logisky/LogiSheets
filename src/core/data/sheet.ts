...
export class SheetService {
    ...
    setCell(
        ...
        if (cell.style) c.setStyle(cell.style)
        if (cell.value) c.value = StandardValue.from(cell.value)
        if (cell.formula !== undefined) c.formula = cell.formula
    ...
    setRowInfo(row: number, info: RowInfo, sheet: number) {
        ...
        if (!s) return
        if (oldRowInfo) s.height += rowInfo.px - oldRowInfo.px
        else s.height += rowInfo.px
    ...
    setColInfo(col: number, info: ColInfo, sheetIndex: number) {
        ...
        if (!sheet) return
        if (oldColInfo) sheet.width + (colInfo.px - oldColInfo.px)
        else sheet.width += colInfo.px
