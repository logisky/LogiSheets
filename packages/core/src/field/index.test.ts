...
function portFrom(grid: Record<string, unknown>) {
    return {
        getValue: (s: number, r: number, c: number) =>
...
describe('checkFieldConstraints', () => {
    it('flags required empties and duplicate uniques', () => {
        ...
        const out = checkFieldConstraints(port, [
    ...
    it('treats empty string as empty for required', () => {
        const port = portFrom({'0:0:0': {type: 'str', value: ''}})
        const out = checkFieldConstraints(port, [
            {
