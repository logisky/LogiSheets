...
export const linkableBlocks: Tool<{sheetIdx: number; colCnt: number}, unknown> =
    {
        ...
        inputSchema: {
            properties: {
                ...
                colCnt: {
                    type: 'integer',
                    description: 'Column count the source range will have.',
                },
            },
        ...
        handler: async (input, ctx) => {
            ...
            if (isErrorMessage(res))
                throw new Error(`linkable_blocks: ${res.msg}`)
            ...
            return {
                data: {blocks},
                display: `${blocks.length} linkable block(s)`,
            }
        },
    }
...
> = {
    ...
    inputSchema: {
        properties: {
            sheetIdx: {
                type: 'integer',
                description: 'Sheet holding the source range.',
            },
            masterRow: {
                type: 'integer',
                description: 'Top-left row of the source range (zero-based).',
            },
            masterCol: {
                type: 'integer',
                description:
                    'Top-left column of the source range (zero-based).',
            },
            rowCnt: {type: 'integer', description: 'Source range row count.'},
            colCnt: {
                type: 'integer',
                description:
                    'Source range column count (must match the block).',
            },
            blockId: {type: 'integer', description: 'The backing block.'},
            blockSheetIdx: {
                type: 'integer',
                description:
                    'Sheet of the block, if different from the source.',
            },
        },
        required: [
            'sheetIdx',
    ...
    handler: async (input, ctx) => {
        await commit(
            asClient(ctx),
            {type: 'createLink', value: input},
            'create_link'
        )
        return {
            data: {
                ok: true,
                range: `${a1(input.masterRow, input.masterCol)}:${a1(
                    input.masterRow + input.rowCnt - 1,
                    input.masterCol + input.colCnt - 1
                )}`,
            },
            display: 'Linked',
        }
