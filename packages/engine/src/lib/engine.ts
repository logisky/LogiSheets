...
export class Engine {
    ...
    private _listeners: Map<EngineEventType, Set<EventCallback<any>>> =
        new Map()
    ...
    constructor(config?: Partial<EngineConfig>) {
        ...
        // A `bindFormSchema` observer used to stamp the schema's ref name onto
        // every host FieldInfo of the block, because the host store carried its own
        // copy of it. The projection reads the ref name off the schema that carries
        // it, so there is nothing left to keep in step.
