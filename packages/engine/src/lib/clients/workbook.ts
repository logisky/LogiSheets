...
export class WorkbookClient implements Client {
    ...
    getAllSheetInfo(): Resp<readonly SheetInfo[]> {
        return this._call(MethodName.GetAllSheetInfo) as Resp<
            readonly SheetInfo[]
        >
    }
    ...
    // Formula auditing (Excel "trace precedents/dependents"). Takes a sheet INDEX
    // (what the app has); the worker resolves it to the worksheet.
    getDependents(params: {
        ...
        endRow: number
        endCol: number
    }): Resp<readonly DependentCell[]> {
        return this._call(MethodName.GetDependents, params) as Resp<
            readonly DependentCell[]
        >
    }

    getPrecedents(params: {
        sheetIdx: number
        row: number
        col: number
    ...
    exportBlockData(params: {
        ...
        return this._call(MethodName.ExportBlockData, params) as Resp<
            readonly BlockDataRow[]
        >
    }
    ...
    getTempStatusChanges(): Resp<TempStatusDiff> {
        return this._call(
            MethodName.GetTempStatusChanges
        ) as Resp<TempStatusDiff>
    }
    ...
    mayModifyBlock: Client['mayModifyBlock'] = (params) =>
        this._call(MethodName.MayModifyBlock, params) as ReturnType<
    ...
    registerCellRemovedCallback: Client['registerCellRemovedCallback'] = (
        ...
        this.getCellId({sheetIdx, rowIdx, colIdx}).then((cellId) => {
            ...
            if (arr) arr.push(callback as CellIdCallback)
                ...
                this._cellRemovedCallbacks.set(key, [
                    callback as CellIdCallback,
                ])
            return
        })
    ...
    getRowHeight: Client['getRowHeight'] = () =>
        this._notImplemented('getRowHeight')
    getColWidth: Client['getColWidth'] = () =>
        this._notImplemented('getColWidth')
