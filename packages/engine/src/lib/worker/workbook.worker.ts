...
export class WorkbookWorkerService implements IWorkbookWorker {
    ...
    public getDependents(params: {
        sheetIdx: number
        startRow: number
        startCol: number
        endRow: number
        endCol: number
    }): Result<readonly DependentCell[]> {
        const ws = this.getSheet(params.sheetIdx)
        return ws.getDependents(
            params.startRow,
            params.startCol,
            params.endRow,
            params.endCol
    ...
    public getPrecedents(params: {
        ...
        row: number
        col: number
    }): Result<readonly CellRefRange[]> {
        const ws = this.getSheet(params.sheetIdx)
        return ws.getPrecedents(params.row, params.col)
    }
    ...
    public getComments(params: {sheetIdx: number}): Result<readonly Comment[]> {
    ...
    public getDiyCellIdWithBlockId(params: {
        ...
        const ws = this.workbook.getWorksheetById(params.sheetId)
        return ws.getDiyCellIdWithBlockId(
            params.blockId,
            params.row,
            params.col
        )
        ...
        try {
            switch (m) {
                ...
                case MethodName.GetSheetDimension:
                    ...
                    break
                case MethodName.GetDependents:
                    result = this.getDependents(args)
                    break
