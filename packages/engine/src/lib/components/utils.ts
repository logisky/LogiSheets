...
export const xForColStartUnclamped = (colIdx: number, grid: Grid): number => {
    ...
    if (colIdx < first.idx)
        return (
            xForColStart(first.idx, grid) - (first.idx - colIdx) * first.width
        )
    if (colIdx > last.idx)
...
export const yForRowStartUnclamped = (rowIdx: number, grid: Grid): number => {
    ...
    if (rowIdx < first.idx)
        return (
            yForRowStart(first.idx, grid) - (first.idx - rowIdx) * first.height
        )
    if (rowIdx > last.idx)
        return (
            yForRowEnd(last.idx, grid) + (rowIdx - last.idx - 1) * last.height
        )
