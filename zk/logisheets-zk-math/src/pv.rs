pub fn calc_pv(rate: f64, nper: f64, pmt: f64, fv: f64, beginning: bool) -> f64 {
    if rate == 0.0 {
        return -(fv + pmt * nper);
    }
    let pmt_at_beginning = if beginning { 1. } else { 0. };
    let temp = libm::powf(1. + rate, nper);
    let factor = (1. + rate * pmt_at_beginning) * (temp - 1.) / rate;
    -(fv + pmt * factor) / temp
}
