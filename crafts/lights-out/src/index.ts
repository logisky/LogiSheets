 ...
 * the language (index.html tracks it) pass it; headless callers (tools.ts) let
 * it fall through to whatever the host injected.
 */
 ...
 * target name is taken (the player has their own "Lights Out" sheet) the
 * engine rejects the rename and we keep playing on the sheet we have — a
 * cosmetic tab label is not worth failing a game over.
 */
