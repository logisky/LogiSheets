...
export function createEngine(L: Locale) {
    ...
    const finImpactUpgradeFormula = (line: string, levelsRef: string): string =>
    ...
    const FIN_IMPACT_PER_ROW_FORMULA: Record<string, string> = {
        [FIN_PRODUCTION_LINE_1_COST]:
            finImpactProductionLineCostFormula(PRODUCTION_LINE_1),
        [FIN_PRODUCTION_LINE_2_COST]:
            finImpactProductionLineCostFormula(PRODUCTION_LINE_2),
    ...
    function rolledSupplierStat(
        ...
        return {
            ...
            unitPrice: Math.max(
                1,
                Math.round(base.unitPrice * tilts.priceTilt)
            ),
        }
    }
    ...
    async function getGameStatus(
        ...
        const engineSheetId = await client.getSheetId({
        ...
        if (
            isErrorMessage(constantsInfo) ||
            isErrorMessage(financialStatusInfo)
        )
            return null
        ...
        const gameStateRowIdx =
            CONSTANTS_TABLE.keys.indexOf(CONST_KEY_GAME_STATE)
        const rawGameState = cellValueAsString(
            constantsInfo.cells[
                gameStateRowIdx * constantsInfo.colCnt + roundValueColIdx
            ]?.value
        )
        const gameState: GameState = (rawGameState as GameState) || 'playing'
    ...
    function _shadowIndicatesFailure(v: Value | 'empty' | undefined): boolean {
    ...
    async function newGame(
        ...
        if (isErrorMessage(sheetTxResult))
            throw new Error(
                `createSheet transaction failed: ${JSON.stringify(
                    sheetTxResult
                )}`
            )
        ...
        for (const name of SHEET_ORDER) {
            ...
            if (!info)
                throw new Error(
                    `sheet ${name} not found at idx ${SHEET_IDX[name]}`
                )
        ...
        for (const def of BLOCK_DEFS) {
            ...
            for (let r = masterRow; r < masterRow + rowCnt; r++)
                sheetRows.add(r)
            ...
            if (table === CONSTANTS_TABLE) {
                const valueCol = fields.findIndex(
            ...
            if (supplierBase) {
                const yieldCol = fields.findIndex(
                    (fi) => fi.name === REQUIRED_YIELD_RATE
                )
                const priceCol = fields.findIndex(
                    (fi) => fi.name === UNIT_PRICE
                )
        ...
        if (isErrorMessage(blockTxResult))
            throw new Error(
                `createBlock transaction failed: ${JSON.stringify(
                    blockTxResult
                )}`
            )
     ...
     */
    async function tick(client: Client, blockIds: BlockIds): Promise<number> {
    ...
    function generateOrder(
        ...
        const multiplier =
            ...
            Math.floor(
                rng() * (profile.penaltyMultMax - profile.penaltyMultMin + 1)
            )
    ...
    async function insertOrder(
        ...
        if (isErrorMessage(tx))
            throw new Error(
                `insertOrder transaction failed: ${JSON.stringify(tx)}`
            )
    ...
    async function watchByBlockRef(
        ...
        if (isErrorMessage(cellId))
            throw new Error(
                `getCellIdByBlockRef failed: ${JSON.stringify(cellId)}`
            )
    ...
    function installJointResearchBridge(client: Client): void {
        ...
        const yieldCol = SUPPLIER_FIELDS.findIndex(
            (f) => f.name === REQUIRED_YIELD_RATE
        )
    ...
    async function advanceRound(
        ...
        {
            const sheetIdForCheck = await client.getSheetId({
                sheetIdx: mainSheetIdx,
            })
            if (isErrorMessage(sheetIdForCheck))
                throw new Error(
                    `getSheetId(MAIN) failed: ${JSON.stringify(
                        sheetIdForCheck
                    )}`
                )
            ...
            if (plMaxColIdxPre >= 0 && plcCapColIdxPre >= 0) {
                ...
                for (let r = 0; r < plcInfoPre.rowCnt; r++) {
                    ...
                    const cap = numericCellValue(
                        plcInfoPre.cells[
                            r * plcInfoPre.colCnt + plcCapColIdxPre
                        ]?.value
                    )
        ...
        if (isErrorMessage(constantsInfo))
            throw new Error(
                `getBlockInfo(constants) failed: ${JSON.stringify(
                    constantsInfo
                )}`
            )
        ...
        const orderStatusDelta =
            orderStatusRowCntAfter - orderStatusRowCntBefore
            ...
            for (let lineRow = 0; lineRow < lineRowCnt; lineRow++) {
                const capacity = numericCellValue(
                    productionLineContribInfo.cells[
                        lineRow * productionLineContribInfo.colCnt +
                            plcCapacityCol
                    ]?.value
                )
        ...
        const supYieldCol = SUPPLIER_FIELDS.findIndex(
            (fi) => fi.name === REQUIRED_YIELD_RATE
        )
        const supPriceCol = SUPPLIER_FIELDS.findIndex(
            (fi) => fi.name === UNIT_PRICE
        )
