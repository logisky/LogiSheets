    ...
    pub fn execute<C: RangeExecCtx>(self, ctx: &C, payload: EditPayload) -> Result<Self, Error> {
        match payload {
            ...
            EditPayload::CreateLink(p) => {
                ...
                let s0 = ctx.fetch_norm_cell_id(&sheet_id, p.master_row, p.master_col)?;
                let s1 = ctx.fetch_norm_cell_id(
                    &sheet_id,
                ...
                let mut exec = self;
                // Register the link. Resolution (record column → block, or invalid
                // reference → #VALUE) happens at calc/dependency time against this
                // links map — the formulas keep their literal ranges. So we just
